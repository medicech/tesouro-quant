...
# Importando os dados reais do sistema
...
# --- FUNÇÃO QUE CAÇA O MODELO CERTO ---
...
# --- 🧠 FILTRO DE SANIDADE (CORREÇÃO DE DADOS) ---
def preparar_contexto_dados():
    try:
        ...
        # 1. Se for IPCA+ e a taxa for > 9%, provavelmente é erro (é prefixado). Vamos remover para não confundir a IA.
        #    (Ou você poderia corrigir o nome, mas remover é mais seguro agora)
        mask_erro_ipca = (df_ia['tipo_titulo'].str.contains('IPCA')) & (df_ia['taxa_compra'] > 9.0)
        df_ia = df_ia[~mask_erro_ipca] # Remove as linhas com erro

        return df_ia.to_string(index=False)
    except:
        return "Erro ao carregar dados de mercado."

# --- PROMPT REFINADO (LÓGICA TEMPORAL) ---
